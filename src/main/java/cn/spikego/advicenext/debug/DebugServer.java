package cn.spikego.advicenext.debug;
...
import cn.spikego.advicenext.features.value.ModeSetting;
import cn.spikego.advicenext.features.value.StringSetting;
...
public class DebugServer {
    ...
    private String getScriptsJson() {
        ...
        try {
            ...
            List<cn.spikego.advicenext.script.Script> scripts = ScriptManager.getInstance().getLoadedScripts();

            for (cn.spikego.advicenext.script.Script script : scripts) {
                if (!first) json.append(",");
                first = false;
                ...
                for (AbstractSetting<?> setting : module.settings) {
                    if (setting.getName().equals(settingName)) {
                        if (setting instanceof BooleanSetting) {
                            ...
                            try {
                                if (setting instanceof cn.spikego.advicenext.features.value.slider.IntSetting) {
                                    ((cn.spikego.advicenext.features.value.slider.IntSetting) setting).setValue(Integer.parseInt(value));
                                } else if (setting instanceof cn.spikego.advicenext.features.value.slider.FloatSetting) {
                                    ((cn.spikego.advicenext.features.value.slider.FloatSetting) setting).setValue(Float.parseFloat(value));
                                } else if (setting instanceof cn.spikego.advicenext.features.value.slider.DoubleSetting) {
                                    ((cn.spikego.advicenext.features.value.slider.DoubleSetting) setting).setValue(Double.parseDouble(value));
                                }
